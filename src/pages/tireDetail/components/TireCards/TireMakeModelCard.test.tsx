...
import { TireMakeModelCard } from './TireMakeModelCard';
...
describe('TireMakeModelCard', () => {
  ...
  it('renderiza marca, modelo, sulcos e profundidade corretamente', () => {
    mockedUseTire.mockReturnValue({
      tire: {
        ...
        disposal: {disposalReasonId:1, disposalImagesUrl:[],disposalReasonDescription: 'qualquer' }, // necessário para passar no if
    ...
    expect(screen.getByText(/Modelo:/)).toHaveTextContent('Modelo: Cinturato P1');
    expect(screen.getByText(/Sulcos:/)).toHaveTextContent('Sulcos: 4');
    expect(screen.getByText(/Profundidade:/)).toHaveTextContent('Profundidade: 7.5 mm');
  });
  ...
  it('renderiza com valores ausentes como texto vazio ou undefined', () => {
    ...
    expect(screen.getByText(/Profundidade:/)).toHaveTextContent('Profundidade: mm');
  });
});
